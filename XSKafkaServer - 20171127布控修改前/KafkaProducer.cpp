...
//初始化Kafka资源
int CKafkaProducer::InitKafkaProducer(const char * pLWKafkaAddress, const char * pSnapTopic, const char * pAlarmTopic, const char * pLayoutLibTopic)
{
    //35.24.22.158，35.24.22.159，35.24.22.142
    //sprintf(m_pBrokers, "%s", "35.24.22.158:9092");
    //sprintf(m_pTopic, "%s", "XINYI_IMAGE_INFO_TOPIC");

    strcpy(m_pBrokers, pLWKafkaAddress);
    strcpy(m_pTopic, pSnapTopic);
    ...
    *应用程序需要定期调用rd_kafka_poll()来服务排队的发送报告回调函数*/
    ...
    rkt = rd_kafka_topic_new(rk, m_pTopic, NULL);
    if (!rkt)
    {
...
//发送消息
int CKafkaProducer::SendProducerMessage(char * pMsg, int nLen)
{
    const void * p = NULL;
    do 
    {
